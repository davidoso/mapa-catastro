<script>
    ...
    // More constants, extracted from the function
    const k0    = 0.9996;                      // Scale on central
    const b     = a * (1 - f);                  // Polar axis
    const e     = Math.sqrt(1 - (b/a)*(b/a));   // Eccentricity
    const e0    = e / Math.sqrt(1 - e*e);       // Called e' in reference
    const esq   = (1 - (b/a) * (b/a));          // e^2 for use in expansions
    const e0sq  = e*e / (1 - e*e);              // e0^2 - always even powers

    function utm2dec(x, y, utmz, north) {
        ...
        if (x > 10000000 || y > 10000000) {
            alert("Nothing may not exceed 10,000,000.");
            return;
        }
        ...
        // Called e1 in USGS PP 1395
        const M0    = 0;  // In case origin other than zero lat - not needed
        ...
        let phi1 = mu + e1 * (3/2 - 27*e1*e1/32) * Math.sin(2*mu) + e1 * e1 * (21/16-55*e1*e1/32) * Math.sin(4*mu);
        ...
        const lng = D * (1 + D * D * ((-1 -2*T1 -C1)/6 + D * D *
